angleData = readtable("C:\Users\Christian Lykke\Documents\MATLAB\vehicleSpinningRightMaxSpeedDataBest.xlsx");
...
angleDegrees = table2array(angleData(2:end, 4)); % Extract angle in degrees as an array
...
title('Angle Over Time Rotating Right At Max Speed');
...
xlabel('Time (s)');
ylabel('Angle Difference (degrees)');
title('Angle Difference Over Time Rotating Right At Max Speed');
xticks(min(timeSteps):0.1668:max(timeSteps));
yticks(-1.5:0.1:7);
xlim([0 max(timeSteps)]);
grid on;